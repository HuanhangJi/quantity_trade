代码：
CAPM为CAPM模型代码
Fama French因子实际所得数据是通过regression_ff_model.py计算得出
calculate_factor_ff_model.py是在只有原始数据没有因子数据时使用的代码
由于这次数据可以直接找到，因此采用的是直接进行回归的regression_ff_model.py


股票数据：
来自网易财经，均采用收盘价，格式为“股票代码_close”，“shanghai_close”为上证指数。
